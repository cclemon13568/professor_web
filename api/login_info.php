<?php
session_start();
include('../config/db.php');
header('Content-Type: application/json; charset=utf-8');

require_once('../libs/PHPMailer/src/PHPMailer.php');
require_once('../libs/PHPMailer/src/SMTP.php');
require_once('../libs/PHPMailer/src/Exception.php');

use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception;

$method = $_SERVER['REQUEST_METHOD'];

switch ($method) {
    case 'POST': // 註冊＋寄信
        $inputJSON = file_get_contents('php://input');
        $input = json_decode($inputJSON, true);

        $teacher_ID = $input['teacher_ID'] ?? '';
        $username = $input['professor_accountnumber'] ?? '';
        $password = $input['professor_password'] ?? '';

        if (empty($teacher_ID) || empty($username) || empty($password)) {
            echo json_encode(["success" => false, "message" => "請輸入教師代號、帳號與密碼"]);
            exit;
        }

        // 查 teacher_ID 是否存在
        $sql = "SELECT teacher_email FROM personal_info WHERE teacher_ID = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $teacher_ID);
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows !== 1) {
            echo json_encode(["success" => false, "message" => "教師代號不存在"]);
            exit;
        }

        $row = $result->fetch_assoc();
        $teacher_email = $row['teacher_email'];

        // 檢查帳號是否已被使用
        $check_account_sql = "SELECT * FROM login_info WHERE professor_accountnumber = ?";
        $check_account_stmt = $conn->prepare($check_account_sql);
        $check_account_stmt->bind_param("s", $username);
        $check_account_stmt->execute();
        $account_result = $check_account_stmt->get_result();

        // 檢查密碼是否已被使用
        $check_password_sql = "SELECT * FROM login_info WHERE professor_password = ?";
        $check_password_stmt = $conn->prepare($check_password_sql);
        $check_password_stmt->bind_param("s", $password);
        $check_password_stmt->execute();
        $password_result = $check_password_stmt->get_result();

        $account_exists = ($account_result->num_rows > 0);
        $password_exists = ($password_result->num_rows > 0);

        if ($account_exists && $password_exists) {
            echo json_encode(["success" => false, "message" => "此帳號密碼已被使用"]);
            exit;
        } elseif ($account_exists) {
            echo json_encode(["success" => false, "message" => "此帳號已被使用"]);
            exit;
        } elseif ($password_exists) {
            echo json_encode(["success" => false, "message" => "此密碼已被使用"]);
            exit;
        }

        // 產生驗證碼
        $verification_code = "稍後會寄出";

        // 寫入 login_info 表
        $insert_sql = "INSERT INTO login_info (professor_accountnumber, professor_password, email, verification_code, teacher_ID)
                    VALUES (?, ?, ?, ?, ?)";
        $insert_stmt = $conn->prepare($insert_sql);
        $insert_stmt->bind_param("sssss", $username, $password, $teacher_email, $verification_code, $teacher_ID);
        $insert_stmt->execute();

        // 寄送驗證碼
        $mail = new PHPMailer(true);
        try {
            $mail->isSMTP();
            $mail->Host = 'smtp.gmail.com';
            $mail->SMTPAuth = true;
            $mail->Username = 'lyfish0316@gmail.com';
            $mail->Password = 'jzbb uoex awqk azsi';
            $mail->SMTPSecure = 'tls';
            $mail->Port = 587;
            $mail->CharSet = 'UTF-8';
            $mail->Encoding = 'base64';

            $mail->setFrom('lyfish0316@gmail.com', '教授系統');
            $mail->addAddress($teacher_email);
            $mail->Subject = "教授系統註冊驗證碼";
            $mail->Body = "您好，您的註冊驗證碼$verification_code\n";

            $mail->send();
            $_SESSION['pending_user'] = $username;
            echo json_encode(["success" => true, "message" => "註冊成功，請查看信箱"]);
        } catch (Exception $e) {
            echo json_encode(["success" => false, "message" => "寄送失敗：" . $mail->ErrorInfo]);
        }
        break;


    case 'GET':
        $teacher_ID = $_GET['teacher_ID'] ?? '';

        if (empty($teacher_ID)) {
            // 沒有帶 teacher_ID，查詢所有帳號
            $sql = "SELECT * FROM login_info";
            $result = $conn->query($sql);

            if ($result->num_rows === 0) {
                echo json_encode(["success" => false, "message" => "目前沒有帳號資料"]);
            } else {
                $data = [];
                while ($row = $result->fetch_assoc()) {
                    $data[] = $row;
                }
                echo json_encode(["success" => true, "data" => $data]);
            }
        } else {
            // 有帶 teacher_ID，查詢單一帳號
            $sql = "SELECT * FROM login_info WHERE teacher_ID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("s", $teacher_ID);
            $stmt->execute();
            $result = $stmt->get_result();

            if ($result->num_rows === 0) {
                echo json_encode(["success" => false, "message" => "找不到帳號"]);
            } else {
                $data = $result->fetch_assoc();
                echo json_encode(["success" => true, "data" => $data]);
            }
        }
        break;


    case 'PUT': // 修改帳密
        $inputJSON = file_get_contents('php://input');
        $input = json_decode($inputJSON, true);
        $current_account = $input['current_account'] ?? ''; // 原本帳號
        $current_password = $input['current_password'] ?? ''; // 原本密碼
        $new_account = $input['new_account'] ?? '';
        $new_password = $input['new_password'] ?? '';

        $missing = [];

        if (empty($current_account)) $missing[] = "目前帳號";
        if (empty($current_password)) $missing[] = "目前密碼";
        if (empty($new_account)) $missing[] = "新帳號";
        if (empty($new_password)) $missing[] = "新密碼";

        if (!empty($missing)) {
            echo json_encode([
                "success" => false,
                "message" => "請提供：" . implode('、', $missing)
            ]);
            exit;
        }


        // 檢查新帳號是否被其他人使用
        $check_account_sql = "SELECT * FROM login_info WHERE professor_accountnumber = ? AND professor_accountnumber != ?";
        $check_account_stmt = $conn->prepare($check_account_sql);
        $check_account_stmt->bind_param("ss", $new_account, $current_account);
        $check_account_stmt->execute();
        $account_result = $check_account_stmt->get_result();
        $account_exists = $account_result->num_rows > 0;

        // 檢查新密碼是否被其他帳號使用
        $check_password_sql = "SELECT * FROM login_info WHERE professor_password = ? AND professor_accountnumber != ?";
        $check_password_stmt = $conn->prepare($check_password_sql);
        $check_password_stmt->bind_param("ss", $new_password, $current_account);
        $check_password_stmt->execute();
        $password_result = $check_password_stmt->get_result();
        $password_exists = $password_result->num_rows > 0;

        if ($account_exists && $password_exists) {
            echo json_encode(["success" => false, "message" => "此帳號與密碼已被使用"]);
            exit;
        } elseif ($account_exists) {
            echo json_encode(["success" => false, "message" => "此帳號已被使用"]);
            exit;
        } elseif ($password_exists) {
            echo json_encode(["success" => false, "message" => "此密碼已被使用"]);
            exit;
        }

        // 更新帳號與密碼
        $update_sql = "UPDATE login_info SET professor_accountnumber = ?, professor_password = ? WHERE professor_accountnumber = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("sss", $new_account, $new_password, $current_account);
        if ($update_stmt->execute()) {
            echo json_encode(["success" => true, "message" => "帳號與密碼已更新"]);
        } else {
            echo json_encode(["success" => false, "message" => "更新失敗"]);
        }
        break;


    case 'DELETE': // 刪除帳號
    $inputJSON = file_get_contents('php://input');
    $input = json_decode($inputJSON, true);
    $username = $input['professor_accountnumber'] ?? '';

    if (empty($username)) {
        echo json_encode(["success" => false, "message" => "帳號不得為空"]);
        exit;
    }

    // 檢查帳號是否存在
    $check_sql = "SELECT * FROM login_info WHERE professor_accountnumber = ?";
    $check_stmt = $conn->prepare($check_sql);
    $check_stmt->bind_param("s", $username);
    $check_stmt->execute();
    $check_result = $check_stmt->get_result();

    if ($check_result->num_rows === 0) {
        echo json_encode(["success" => false, "message" => "找不到 accountnumber 為 $username 的帳號"]);
        exit;
    }

    // 刪除帳號
    $sql = "DELETE FROM login_info WHERE professor_accountnumber = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $username);
    $stmt->execute();

    echo json_encode(["success" => true, "message" => "帳號已刪除"]);
    break;


    default:
        echo json_encode(["success" => false, "message" => "不支援的請求方式"]);
        break;
}
?>
